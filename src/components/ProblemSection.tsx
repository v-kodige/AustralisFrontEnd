    ...
    <section className="py-16 bg-australis-lightGray" id="problem">
      <div className="container-custom">
        <div className="flex flex-col items-center justify-center gap-2 mb-10">
          ...
          <h2 className="text-2xl font-semibold text-center">Focus on what works. Skip what doesn't.</h2>
        ...
        <div ref={sectionRef} className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
          {stats.map((stat, index) => (
            <div 
              key={index}
              className={`glass-card bg-white/30 backdrop-blur-sm p-8 rounded-xl shadow-sm border border-white/20 hover:shadow-md transition-shadow transform ${inView ? 'animate-fade-in' : 'opacity-0'}`}
